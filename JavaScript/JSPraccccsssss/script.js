...
const showResult = (arr)=>{
    const parent = document.querySelector(".result-content");
    parent.innerHTML = "";
...
const getData = (searchText)=>{
    ...
    const pr = window.fetch(`
        https://api.geoapify.com/v1/geocode/autocomplete?text=${searchText}&type=city&limit=6&format=json&filter=countrycode:in&apiKey=${API_KEY}
    `);
...
// let data = [];
const cardInfo = localStorage.getItem("card-info");
const data = cardInfo ? JSON.parse(cardInfo) : [];
